# visualization/late_slope.py
# 图3：后期稳定化分布（LateSlope & LateMean）。
# 用箱线图对比正确组和错误组在维度 3 后期指标上的分布差异：
#   LateSlope：后 k 层 UpdateNorm 的线性回归斜率（越负 = 越快收敛）
#   LateMean： 后 k 层 UpdateNorm 的均值（越小 = 越稳定）
#
# 默认主分析对象为整段生成内容。

import matplotlib.pyplot as plt

from config import FIGURES_DIR, LATE_K


def plot_late_slope_distribution(sample_records):
    FIGURES_DIR.mkdir(exist_ok=True)

    non_hallucinated = [r for r in sample_records if not r["has_hallucination"]]
    hallucinated = [r for r in sample_records if r["has_hallucination"]]

    if not non_hallucinated or not hallucinated:
        print("[fig3] Not enough samples. Skipping.")
        return

    nh_slopes = [r["update_norm_late_slope"] for r in non_hallucinated]
    h_slopes = [r["update_norm_late_slope"] for r in hallucinated]
    nh_means = [r["update_norm_late_mean"] for r in non_hallucinated]
    h_means = [r["update_norm_late_mean"] for r in hallucinated]

    fig, axes = plt.subplots(1, 2, figsize=(10, 5))
    fig.suptitle(f"Dim 3: Late-Layer Stabilization  (Whole Response Average, last {LATE_K} layers)", fontsize=13)

    axes[0].boxplot([nh_slopes, h_slopes], labels=["Non-H", "Hall"])
    axes[0].set_title("LateSlope of UpdateNorm")
    axes[0].set_ylabel("Slope  (more negative → faster convergence)")
    axes[0].grid(True, alpha=0.3)

    axes[1].boxplot([nh_means, h_means], labels=["Non-H", "Hall"])
    axes[1].set_title("LateMean of UpdateNorm")
    axes[1].set_ylabel("Mean ‖Δh‖ in late layers")
    axes[1].grid(True, alpha=0.3)

    plt.tight_layout()
    path = FIGURES_DIR / "fig3_late_slope.png"
    plt.savefig(path, dpi=150)
    plt.close()
    print(f"Saved {path}")
